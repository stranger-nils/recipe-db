      ...
      <div className="rail-items">
        {items.map((it) => (
          <button key={it.id} className={cx("rail-btn", mode === it.id && "active")}
            ...
            <Ic name={it.icon} size={21} stroke={1.25} />
...
function Explorer({ mode, db, grouping, setGrouping, query, setQuery, activeKey,
  ...
  const toggleGroup = (k) => setCollapsed((s) => {
  ...
  if (mode === "planering") {
    ...
    return (
      <aside className="explorer">
        ...
        <div className="exp-scroll">
          {groups.map((g) => {
            const gk = `plan:${g.key}`; const col = collapsed.has(gk);
            return (
            <div key={g.key} className={cx("tree-group", col && "collapsed")}>
              <button className="tree-group-h" onClick={() => toggleGroup(gk)}>
                <span className="tg-chev"><Ic name="chevron" size={11} /></span>
                <span>{g.key}</span>
  ...
  return (
    <aside className="explorer">
      ...
      <div className="exp-scroll">
        {groups.map((g) => {
          const gk = `rec:${g.key}`; const col = collapsed.has(gk);
          return (
          <div key={g.key} className={cx("tree-group", col && "collapsed")}>
            <button className="tree-group-h" onClick={() => toggleGroup(gk)}>
              <span className="tg-chev"><Ic name="chevron" size={11} /></span>
...
function App() {
  ...
  return (
    <div className={cx("ide", `dens-${t.density}`, `ide-${t.ide}`, `case-${t.case}`)}
      style={{ "--serif": (FONT_PAIRS[t.fontPair] || FONT_PAIRS.plex).serif, "--mono": (FONT_PAIRS[t.fontPair] || FONT_PAIRS.plex).mono, "--accent": t.accent }}>
      ...
      <TweaksPanel>
        ...
        <TweakSelect label="Typsnitt" value={t.fontPair}
          options={[{ value: "plex", label: "IBM Plex Mono" },
            { value: "jetbrains", label: "JetBrains Mono" },
            { value: "spacemono", label: "Space Mono" }]}
          onChange={(v) => setTweak("fontPair", v)} />
        <TweakRadio label="Versaler" value={t.case}
          options={["versaler", "normal", "gemener"]} onChange={(v) => setTweak("case", v)} />
        ...
        <TweakColor label="Accent" value={t.accent}
          options={["#0f6f86", "#16859d", "#1f9ab4", "#2ab0c9", "#0d8267"]}
