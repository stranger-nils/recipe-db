...
if [ "$CODE" != "200" ]; then
  ...
  docker compose -f docker-compose.yml -f docker-compose.rpi.yml logs --tail 40 >&2
  exit 1
fi
...
echo "Ångra: docker compose -f docker-compose.yml -f docker-compose.rpi.yml down"